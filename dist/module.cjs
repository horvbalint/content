...
const abstractTree_js = require('../dist/runtime/internal/abstract-tree.js');
...
async function getLocalDatabase(database, { connector, sqliteConnector } = {}) {
  ...
  const cacheCollection = {
    ...
    extendedSchema: z$1.z.object({
      id: z$1.z.string(),
      value: z$1.z.string(),
      checksum: z$1.z.string()
    }),
    fields: {
      id: "string",
      value: "string",
...
const markdown = defineTransformer({
  ...
  parse: async (file, options = {}) => {
    ...
    if (options.compress) {
      return {
        ...parsed.data,
        excerpt: parsed.excerpt ? abstractTree_js.compressTree(parsed.excerpt) : void 0,
        body: {
  ...
  getContents: ({ options }) => {
    ...
    return [
      ...
      ...publicCollections.map(
        (c) => indentLines(`interface ${scule.pascalCase(c.name)}CollectionItem extends ${parentInterface(c)} ${zodToTs.printNode(zodToTs.zodToTs(c.schema, scule.pascalCase(c.name)).node)}`)
      ),
      "",
...
function generateJsonSchema(collection) {
  const jsonSchema = zodToJsonSchema.zodToJsonSchema(collection.extendedSchema, collection.name);
...
async function watchContents(nuxt, options, manifest, socket) {
  ...
  const sourceMap = collections.flatMap((c) => {
    if (c.source) {
      return c.source.filter((s) => !s.repository).map((s) => ({ collection: c, source: s, cwd: s.cwd && ufo.withTrailingSlash(s.cwd) }));
    }
    return [];
  });
...
function getUnderlyingTypeName(zodType) {
...
function defineCollection(collection) {
  ...
  if (collection.type === "page") {
    schema = pageSchema.extend(schema.shape);
  }
  schema = metaSchema.extend(schema.shape);
  return {
    type: collection.type,
...
function generateCollectionTableDefinition(collection, opts = {}) {
  ...
  const sqlFields = sortedKeys.map((key) => {
    ...
    let sqlType = ZodToSqlFieldTypes[underlyingType.constructor.name];
    ...
    if (underlyingType.constructor.name === "ZodString") {
      const checks = underlyingType._def.checks || [];
    ...
    if (type._def.defaultValue !== void 0) {
      let defaultValue = typeof type._def.defaultValue() === "string" ? `'${type._def.defaultValue()}'` : type._def.defaultValue();
