...
import { compressTree } from '../dist/runtime/internal/abstract-tree.js';
...
const markdown = defineTransformer({
  ...
  parse: async (file, options = {}) => {
    ...
    if (options.compress) {
      return {
        ...parsed.data,
        excerpt: parsed.excerpt ? compressTree(parsed.excerpt) : void 0,
        body: {
  ...
  getContents: ({ options }) => {
    ...
    return [
      ...
      ...publicCollections.map(
        (c) => indentLines(`interface ${pascalCase(c.name)}CollectionItem extends ${parentInterface(c)} ${printNode(zodToTs(c.schema, pascalCase(c.name)).node)}`)
      ),
      "",
